...
json.dump(record, (sweep_root / f"{tag}.summary.json").open("w", encoding="utf-8"), ensure_ascii=False, indent=2)
...
records = [json.load(p.open("r", encoding="utf-8")) for p in sorted(root.glob("t*.summary.json"))]
...
while IFS= read -r full_cfg; do
  [[ -n "${full_cfg}" ]] || continue
  python main.py --config "${full_cfg}" 2>&1 | tee "${FULL_ROOT}/$(basename "${full_cfg}" .json).log"
